...
@section('content')
    <div class="main-content">
        <div class="contents">
            <form method="post" action="{{ route('productos.update', $producto->id) }}" class="row g-3"
                                ...
                                <div class="card-body">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" class="form-control" value="{{ $producto->nombre }}"
                                            name="nombre" id="nombre" placeholder="Shirt, t-shirts, etc."
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="mb-4">
                                                ...
                                                <input type="text" class="form-control" value="{{ $producto->sku }}"
                                                    name="sku" id="sku">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="mb-4">
                                                ...
                                                <div class="input-group">
                                                    <input type="text" class="form-control"
                                                        value="{{ $producto->weight }}" name="weight" id="weight"
                                    ...
                                    <textarea name="descripcion" id="descripcion" class="ckeditor" rows="10" cols="80" required>{!! $producto->descripcion !!}</textarea>
                                </div>
                            </div>
                                    ...
                                    <label for="dropzone" class="file-upload__label">
                                        <div class="upload-product-img px-10 d-block">
                                            <div id="dropzone">
                                                ...
                                                <!-- Agrega esta sección dentro del formulario de edición -->
                                                <div class="form-group">
                                                    <label for="existing-images">Imágenes existentes:</label>
                                            ...
                                            <table class="table table-striped table-hover">
                                                ...
                                                <tr>
                                                    <td class="image-cell">
                                                        ...
                                                        <!-- Puedes usar $variante->imagen para obtener la ruta de la imagen -->
                                                    </td>
                                                    <td>{{ $variante->combination }}</td>
                                                    <td><input type="number" name="variantes[{{ $index }}][precio]" class="form-control" value="{{ $variante->precio }}"></td>
                                                    <td><input type="number" name="variantes[{{ $index }}][stock]" class="form-control" value="{{ $variante->stock }}"></td>
                                                    <td><input type="number" name="variantes[{{ $index }}][stock2]" class="form-control" value="{{ $variante->stock2 }}"></td>
                                                    <td><input type="text" name="variantes[{{ $index }}][sku]" class="form-control" value="{{ $variante->sku }}"></td>
                                                    <td><input type="number" name="variantes[{{ $index }}][peso]" class="form-control" value="{{ $variante->peso }}"></td>
                                                    <td>
                                                        <select name="variantes[{{ $index }}][estado]" class="form-control">
                                                            <option value="existente" {{ $variante->estado === 'existente' ? 'selected' : '' }}>Existente</option>
                                                            <option value="agotado" {{ $variante->estado === 'agotado' ? 'selected' : '' }}>Agotado</option>
                                                        </select>
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        <div class="col-lg-4 mb-25 col-md-6">
                            <div class="card card-default card-md mb-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="precio" id="precio"
                                                placeholder="0.00" aria-label="0.00" value="{{ $producto->precio }}">
                                        </div>
                                    </div>
                                    ...
                                    <label for="ganancia" class="form-label">Profit</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="ganancia" placeholder="0.00"
                                            disabled>
                                    </div>

                                    <label for="margen" class="form-label mt-3">Profit Margin (%)</label>
                                    <div class="input-group">
...
@section('js')
    ...
    <script>
        // Asegúrate de que este script se ejecute después de que el DOM haya cargado
        document.addEventListener("DOMContentLoaded", function() {
            var existingImagesContainer = document.getElementById("existing-images-container");

            // Obtener la información de las imágenes existentes desde Blade (PHP)
            var existingImagesInfo = @json($producto->imagenes);

            // Verificar si el contenedor existe y si hay imágenes existentes
            if (existingImagesContainer && existingImagesInfo.length > 0) {
                // Iterar sobre los nombres de las imágenes existentes
                existingImagesInfo.forEach(function(imageName) {
                    // Crear la URL de la imagen existente
                    var imgSrc = "/img/post/" + imageName;
                    var existingImagePreview = createPreview(imgSrc, imageName);

                    // Agregar la vista previa al contenedor principal
                    existingImagesContainer.appendChild(existingImagePreview);
                });
            }

            function createExistingImagePreview(imgSrc, fileName) {
                var existingImagePreview = document.createElement("div");
                existingImagePreview.classList.add("existing-image-preview", "mt-25");
                ...
                var img = document.createElement("img");
                img.src = imgSrc;
                img.alt = fileName; // Cambiado a usar el nombre del archivo como atributo alt
                img.style.width = "100px"; // Tamaño fijo de la imagen
                img.style.height = "80px"; // Tamaño fijo de la imagen
                img.style.aspectRatio = "3/2"; // Aspect ratio 3/2
                img.style.objectFit = "contain"; // object-fit: contain
                ...
                deleteBtn.addEventListener("click", function() {
                    // Aquí puedes agregar la lógica para eliminar la imagen existente en el servidor o marcarla para eliminación
                    // También puedes ocultar la vista previa en lugar de eliminarla completamente
                    // Ejemplo: existingImagePreview.remove();
                });

                deleteBtnContainer.appendChild(deleteBtn);
                imgContainer.appendChild(deleteBtnContainer);

                existingImagePreview.appendChild(imgContainer);
                existingImagePreview.appendChild(infoContainer);

                return existingImagePreview;
            }
